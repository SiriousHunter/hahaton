...
async function mongooseConnector(
        ...
        models.forEach(model => {
            ...
            if (model?.indexes?.length) {
                for (const index of model?.indexes) {
                    index.schema && schema.index(index.schema, index.options);
                }
            }

            model.class && schema.loadClass(model.class);

            model.virtualize && model.virtualize(schema);

            if (useNameAndAlias) {
                /* istanbul ignore next */
                if (model.alias === undefined) throw new Error(`No alias defined for ${model.name}`);
    ...
    await mongoose.syncIndexes().catch(console.error);
